...
def get_cifar100_coarse(
    ...
    shuffled = list(trainset["groups"].keys())
    np.random.shuffle(shuffled)
    shuffled = {v: i for i, v in enumerate(shuffled)}
    ...
    for k, v in shuffled.items():
        supercls.append(v)
        class_order.extend(trainset["groups"][k])
    ...
    for i, (image, super_label, sub_label) in enumerate(trainset["data"]):
        ...
        task = shuffled[super_label]
        # data[super_label]['trn']['x'].append(image)
        # data[super_label]['trn']['y'].append(class_order.index(sub_label))
        data[task]['trn']['x'].append(image)
        data[task]['trn']['y'].append(class_order.index(sub_label))
    ...
    if test_embedding_path:
        ...
        for i, (image, super_label, sub_label) in enumerate(testset["data"]):
            ...
            data[task]['tst']['x'].append(image)
            data[task]['tst']['y'].append(class_order.index(sub_label))            
    ...
    if val_embedding_path:
        for i, (image, super_label, sub_label) in enumerate(valset["data"]):
            ...
            data[task]['val']['x'].append(image)
            data[task]['val']['y'].append(class_order.index(sub_label))
    elif validation > 0.0:
        for tt in data.keys():
    ...
    data["ordered"] = {class_order.index(c): v for k, v in shuffled.items() for c in trainset["groups"][k]}
...
def get_data(
    ...
    if 'subclass_targets' in trainset.keys():
        num_classes = len(np.unique(trainset["subclass_targets"]))
    else:
    ...
    if len(trainset["data"][0]) == 3:
        # coarse cifar100
        for i, (this_image, _, this_label) in enumerate(trainset["data"]):
            ...
            # print(f"task: {this_task}\told_label: {old_label}\tnew_label: {this_label}")
            
            data[this_task]['trn']['x'].append(this_image.numpy())
            # data[this_task]['trn']['y'].append(this_label - init_class[this_task]) # this breaks somehow
            data[this_task]['trn']['y'].append(this_label)

            if dummy and i >= 500:
                break       
    else:
        # standard x, y dataset
    ...
    if test_embedding_path:
        if len(testset["data"][0]) == 3:
            # coarse cifar100
            for i, (this_image, _, this_label) in enumerate(testset["data"]):
                this_label = int(this_label)
                this_label = class_order.index(this_label)
                this_task = (this_label >= cpertask_cumsum).sum()

                data[this_task]['tst']['x'].append(this_image.numpy())
    ...
    if val_embedding_path:
        ...
        for i, (this_image, this_label) in enumerate(valset["data"]):
            ...
            data[this_task]['val']['y'].append(this_label)       
    elif validation > 0.0:
        for tt in data.keys():
            pop_idx = [i for i in range(len(data[tt]["trn"]["x"]))]
            val_idx = random.sample(pop_idx, int(np.round(len(pop_idx) * validation)))
