...
class Appr(Inc_Learning_Appr):
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        non_selected = []
        # print(f"t: {t}\ntrn_loader.dataset.labels: {np.unique(trn_loader.dataset.labels)}")
        for curr_cls in range(num_old_cls, num_cls):
            ...
            if isinstance(trn_loader.dataset, base_har.BaseDataset):
                self.x_valid_exemplars[curr_cls] = [trn_loader.dataset.inputs[idx] for idx in cls_ind[:num_val_ex_cls]]
        ...
        if isinstance(trn_loader.dataset, base_har.BaseDataset):
            trn_loader.dataset.inputs = [trn_loader.dataset.inputs[idx] for idx in non_selected]
        ...
        if t > 0:
            ...
            # FR27 added the line below
            # otherwise the combined loader will have mismatch data type: np.array and torch.tensor
            trn_loader.dataset.images = torch.tensor(np.array(trn_loader.dataset.images))
        ...
        if t > 0:
            ...
            # print(type(bic_val_dataset))
            if isinstance(bic_val_dataset, base_har.BaseDataset):
                bic_val_dataset.inputs = sum(self.x_valid_exemplars, [])
            elif isinstance(bic_val_dataset.images, list):
            ...
            for e in range(self.bias_epochs):
                ...
                # print("=====> bic_val_dataset.shape:", len(bic_val_dataset.images))
                # print("=====> bic_val_dataset.labels:", np.unique(bic_val_dataset.labels))
                for inputs, targets in bic_val_loader:
                    ...
                    with torch.no_grad():
                        ...
                        # for outs in outputs:
                        #     print("====> outputs.size():", outs.size())
                        old_cls_outs = self.bias_forward(outputs[:t])
                        # print("====> LEN(old_cls_outs):", len(old_cls_outs))
                        # for outs in old_cls_outs:
                        #     print("====> old_cls_outs.size():", outs.size())
                    new_cls_outs = self.bias_layers[t](outputs[t])
                    # print("====> new_cls_outs.size():", new_cls_outs.size())
                    # for outs in new_cls_outs:
                    #     print("====> new_cls_outs.size():", outs.size())
                    pred_all_classes = torch.cat([torch.cat(old_cls_outs, dim=1), new_cls_outs], dim=1)
                    # print("====> pred_all_classes.size():", pred_all_classes.size())
    ...
    def train_epoch(self, t, trn_loader):
        ...
        # print(dir(trn_loader))
        # for dat in trn_loader.dataset.datasets:
        #     print(type(dat))
        # print(f"task: {t}")
        ...
        for images, targets in trn_loader:
            ...
            if t > 0:
                ...
                # print("\tLEN(targets_old):", len(targets_old))
                # for out_ in targets_old:
                #     print(f"\ttargets_old.size(): {out_.size()}")
                targets_old = self.bias_forward(targets_old)  # apply bias correction
                # for to_ in targets_old:
                #     print(f"\ttargets_old.corrected.size(): {to_.size()}")
            # Forward current model
            outputs = self.model(images.to(self.device))
            # print("LEN(outputs):", len(outputs))
            # for out_ in outputs:
            #     print(f"outputs.size(): {out_.size()}")
            outputs = self.bias_forward(outputs)  # apply bias correction
            # for to_ in outputs:
            #     print(f"outputs.corrected.size(): {to_.size()}")
            # if targets_old:
            #     print("LEN(targets_old) -- redundant:", len(targets_old))
            #     for to_ in targets_old:
            #         print(f"\ttargets_old.corrected.size(): {to_.size()}")
    ...
    def criterion(self, t, outputs, targets, targets_old):
        ...
        # print("\t===CRITERION===")
        if t > 0:
            # print("===CALCULATE CROSSENTROPY WITH TEMPERATURE===")
        ...
        if self.lamb == -1:
            # print(f"\tself.model.task_cls[:{t}]: {self.model.task_cls[:t]}")
            # print(f"\tself.model.task_cls[:{t}].sum(): {self.model.task_cls[:t].sum()}")
            # print("\tself.model.task_cls.sum():", self.model.task_cls.sum())
    ...
    def final_prediction(self, features, targets):
        features = features.to(self.device)
        targets = targets.to(self.device)
        outputs = self.model(features)
        preds = torch.cat(outputs, dim=1).argmax(1)        
